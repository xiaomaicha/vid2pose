...
def _run_inference():
  ...
  with sv.managed_session() as sess:
    ...
    if FLAGS.kitti_video == 'test_files_stereo':
      # im_files_2 = util.read_text_lines(
      #    'dataset/kitti/test_files_stereo.txt')
      # im_files_2 = [os.path.join(FLAGS.kitti_dir, f) for f in im_files_2]
      im_files_2,im_files_3 = util.read_text_lines_stereo('dataset/kitti/kitti_stereo_2015_test_files.txt')
      im_files_2 = [os.path.join(FLAGS.kitti_dir, f) for f in im_files_2]
      im_files_3 = [os.path.join(FLAGS.kitti_dir, f) for f in im_files_3]
      
    else:
      video_path = os.path.join(FLAGS.kitti_dir, FLAGS.kitti_video)
      im_files_2 = gfile.Glob(os.path.join(video_path, 'image_02/data', '*.png'))
      im_files_2 = [f for f in im_files_2 if 'disp' not in f]
      im_files_2 = sorted(im_files_2)

      im_files_3 = gfile.Glob(os.path.join(video_path, 'image_03/data', '*.png'))
      im_files_3 = [f for f in im_files_3 if 'disp' not in f]
      im_files_3 = sorted(im_files_3)

    depth = np.zeros((len(im_files_2), FLAGS.img_height, FLAGS.img_width), dtype=np.float32)
    for i in range(0, len(im_files_2), FLAGS.batch_size):
      if i % 100 == 0:
      ...
      for b in range(FLAGS.batch_size):
        idx = i + b
        if idx >= len(im_files_2):
          break
        im_2 = scipy.misc.imread(im_files_2[idx])
        im_3 = scipy.misc.imread(im_files_3[idx])
        im_left = scipy.misc.imresize(im_2, (FLAGS.img_height, FLAGS.img_width))
        im_right = scipy.misc.imresize(im_3, (FLAGS.img_height, FLAGS.img_width))
        im_stereo = np.concatenate((im_left,im_right), axis=2)
        inputs_left[b] = im_left
        inputs[b] = im_stereo
      ...
      for b in range(FLAGS.batch_size):
        ...
        input_float = inputs_left[b].astype(np.float32) / 255.0

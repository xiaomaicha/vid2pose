...
class DataReader(object):
  ...
  def read_data(self):
    ...
    with tf.name_scope('data_loading'):
      ...
      with tf.name_scope('image_augmentation_flip'):
        image_stack, intrinsics = self.augment_images_flip(image_stack, intrinsics, randomized=random_flipping)

      # with tf.name_scope('image_augmentation_scale_crop'):
      #   do_scale_crop = tf.random_uniform([], 0, 1)
      #   image_stack, intrinsics = tf.cond(do_scale_crop>0.5, lambda:self.augment_images_scale_crop(
      #     image_stack, intrinsics, self.img_height, self.img_width), lambda: (image_stack, intrinsics))
  ...
  def read_data_test(self):
    ...
    with tf.name_scope('data_loading'):
      with tf.name_scope('enqueue_paths'):
        seed = random.randint(0, 2 ** 31 - 1)
        dataset_dir = self.data_dir[:-5] + 'test'
        self.file_lists_test = self.compile_file_list(dataset_dir, 'test')
        image_paths_queue = tf.train.string_input_producer(
          self.file_lists_test['image_file_list'], seed=seed,  capacity = 2048, shuffle=True)
        cam_paths_queue = tf.train.string_input_producer(
          self.file_lists_test['cam_file_list'], seed=seed,  capacity = 2048, shuffle=True)
        img_reader = tf.WholeFileReader()
        _, image_contents = img_reader.read(image_paths_queue)
        image_seq = tf.image.decode_jpeg(image_contents)
      ...
      #   image_seq = tf.cond(do_augment > 0.5, lambda:self.augment_image_colorspace(image_seq), lambda: image_seq)
      with tf.name_scope('image_augmentation'):
        image_seq = self.augment_image_colorspace(image_seq)

      image_stack = self.unpack_images(image_seq)  #1*5
  ...
  def unpack_images(self, image_seq):
    ...
    with tf.name_scope('unpack_images'):
      image_list = [
          image_seq[0:self.img_height, i * self.img_width:(i + 1) * self.img_width, :]
      ...
      image_list_1 = [
        image_seq[self.img_height:self.img_height*2, i * self.img_width:(i + 1) * self.img_width, :]
        for i in range(self.seq_length)
      ]
      image_stack_1 = tf.concat(image_list_1, axis=2)
      image_stack_1.set_shape(
        [self.img_height, self.img_width, self.seq_length * 3])

      image_stack = tf.concat([image_stack,image_stack_1], axis=2)
    return image_stack

  def unpack_image_sequence(self, image_seq):
    num_source = self.seq_length - 1
    image_seq_list = []
    for i in range(num_source):
      for j in range(2):
        slice_image = tf.slice(image_seq, [i * self.img_height, j * self.img_width, 0],
  ...
  def augment_image_colorspace(cls, image_seq):
    ...
    image_seq_aug = image_seq
    # Randomly shift brightness.
    apply_brightness = tf.less(tf.random_uniform(
      shape=[], minval=0.0, maxval=1.0, dtype=tf.float32), 0.5)
    image_seq_aug = tf.cond(
      apply_brightness,
      lambda: tf.image.random_brightness(image_seq_aug, max_delta=0.1),
      lambda: image_seq_aug)
    ...
    image_seq_aug = tf.cond(
      apply_contrast,
      lambda: tf.image.random_contrast(image_seq_aug, 0.85, 1.15),
      lambda: image_seq_aug)
    ...
    image_seq_aug = tf.cond(
      apply_saturation,
      lambda: tf.image.random_saturation(image_seq_aug, 0.85, 1.15),
      lambda: image_seq_aug)
    ...
    image_seq_aug = tf.cond(
      apply_hue,
      lambda: tf.image.random_hue(image_seq_aug, max_delta=0.1),
      lambda: image_seq_aug)
  ...
  @classmethod
  def augment_images_flip(cls, image_stack, intrinsics,
                          randomized=True):
    """Randomly flips the image horizontally."""

    def flip(cls, image_stack, intrinsics):
      _, in_w, in_c = image_stack.get_shape().as_list()
      fx = intrinsics[0, 0]
      fy = intrinsics[1, 1]
      ...
      image_stack_left = tf.image.flip_left_right(image_stack[:, :, tf.cast(in_c/2,dtype=tf.int32):])
      image_stack_right = tf.image.flip_left_right(image_stack[:, :,:tf.cast(in_c/2,dtype=tf.int32)])

      flip_image_stack = tf.concat([image_stack_left, image_stack_right], axis=2)

      return (flip_image_stack, intrinsics)

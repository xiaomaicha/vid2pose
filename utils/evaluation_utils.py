...
def convert_disps_to_depths_kitti(gt_disparities, pred_disparities):
    ...
    for i in range(len(gt_disparities)):
        ...
        # inv_depth_pred
        inv_depth_pred= cv2.resize(pred_disp, (width, height), interpolation=cv2.INTER_LINEAR)
        depth_pred = inv_depth_pred
            # 1.0 / (inv_depth_pred + 1e-4)
        depth_pred[np.isinf(depth_pred)] = 0
        pred_disp_resized = width_to_focal[width] * 0.54 / inv_depth_pred

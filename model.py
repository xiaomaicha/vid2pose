...
class Model(object):
    ...
    def build_inference_for_training(self):
        ...
        with tf.name_scope("data_loading"):
            ...
            intrinsic_mat_input = tf.placeholder(tf.float32, shape=[self.batch_size, NUM_SCALES, 3, 3],
                                                 name='intrincis')
            intrinsic_mat_inv_input = tf.placeholder(tf.float32, shape=[self.batch_size, NUM_SCALES, 3, 3],
                                                     name='intrincis_inv')
        ...
        with tf.name_scope('compute_loss'):
            ...
            with tf.name_scope("sad_kernel"):
                self.sad_loss_kernel_scale_constant = [
                    tf.constant([1.0 / (3 * (7 - s) * (9 - s))], shape=[7 - s, 9 - s, 3, 3])
            ...
            with tf.name_scope('flow_loss'):
                ...
                for step in range(self.max_egomotion_step):
                    ...
                    for i in range(egomotion_num):
                        ...
                        self.direct_flow_fwd[egomotion_index][s] = self.direct_flow_fwd[egomotion_index][
                                                                       s] * scale_factor

                        self.temporal_warp_image_flow[s][key] = project.flow_warp(
                            self.images_left[s][:, :, :, 3 * i:3 * (i + 1)],
                ...
                for step in range(self.max_egomotion_step):
                    ...
                    for j in range(egomotion_num):
                        ...
                        scale_factor = tf.cast(tf.constant([curr_w, curr_h], shape=[1, 1, 1, 2]), 'float32')
                        scale_factor = tf.tile(scale_factor, [curr_bs, curr_h, curr_w, 1])
                        self.direct_flow_bwd[egomotion_index][s] = self.direct_flow_bwd[egomotion_index][
                                                                       s] * scale_factor
    ...
    def temporal_loss(self):
        ...
        if self.fwd_bwd_egomoton_consistency_weight > 0:
            with tf.name_scope('fwd_bwd_egomoton_consistency'):
                tran_rot_tatio = 20
                egomotion_index_base = 0
                for step in range(self.max_egomotion_step):
                    egomotion_num = self.num_source - step
                    ...
                    for i in range(egomotion_num):
                        ...
                        self.temporal_egomotion_fwd_bwd_consistency_loss += \
                            ...
                            + tran_rot_tatio * tf.reduce_mean(egomotion_fwd[:, 3:] + egomotion_bwd[:, 3:])
                ...
                with tf.name_scope('temporal_fwd_loss'):
                    ...
                    for step in range(self.max_egomotion_step):
                        ...
                        for i in range(egomotion_num):
                            ...
                            # 光流光度约束
                            curr_bs, curr_h, curr_w, _ = self.pose_net_flow_fwd[egomotion_index][
                                s].get_shape().as_list()
                            scale_factor = tf.cast(tf.constant([curr_w, curr_h], shape=[1, 1, 1, 2]), 'float32')
                            scale_factor = tf.tile(scale_factor, [curr_bs, curr_h, curr_w, 1])
                            self.pose_net_flow_fwd[egomotion_index][s] = self.pose_net_flow_fwd[egomotion_index][
                                                                             s] * scale_factor
                            ...
                            self.temporal_reconstr_loss_pose_flow += tf.reduce_mean(
                                self.temporal_warp_error_pose_flow[s][key])
                            self.temporal_ssim_loss_pose_flow += tf.reduce_mean(
                                self.temporal_ssim_error_pose_flow[s][key])

                            # 位姿光度约束
                            egomotion = tf.squeeze(self.egomotion_fwd[egomotion_index])
                            self.temporal_warped_image[s][key], self.temporal_warp_mask[s][key], \
                            ...
                            self.temporal_warp_error[s][key], self.temporal_ssim_error[s][
                            ...
                            #   self.temporal_warp_error[s][key] = self.charbonnier_loss(self.temporal_warp_error[s][key])
                            # # sad_loss
                            # if self.sad_loss is True:
                            #   self.temporal_warp_error[s][key] = tf.nn.conv2d(self.temporal_warp_error[s][key], self.sad_loss_kernel_scale[s],
                            ...
                            # # SSIM.
                            # if self.ssim_weight > 0:
                            #   self.temporal_ssim_error[s][key] = self.ssim(self.temporal_warped_image[s][key],
                            #                                       self.images_left[s][:, :, :, 3 * j:3 * (j + 1)])
                            #   # use_charbonnier_loss
                            #   if self.use_charbonnier_loss is True:
                            #     self.temporal_ssim_error[s][key] = self.charbonnier_loss(self.temporal_ssim_error[s][key])
                            #   if self.sad_loss is True:
                            #     self.temporal_ssim_error[s][key] = tf.nn.conv2d(self.temporal_ssim_error[s][key], self.sad_loss_kernel_scale[s],
                            #                                            [1, 1, 1, 1], padding='SAME')
                            #   # TODO(rezama): This should be min_pool2d().
                            #   # ssim_mask = slim.avg_pool2d(self.temporal_warp_mask[s][key], 3, 1, 'VALID')
                            #   if self.use_geometry_mask is True:
                            #     self.temporal_ssim_error[s][key] = self.temporal_ssim_error[s][key] * slim.avg_pool2d(self.temporal_warp_mask[s][key], 3, 1,
                            #                                                                         'VALID')
                ...
                with tf.name_scope('temporal_bwd_loss'):
                    ...
                    for step in range(self.max_egomotion_step):
                        ...
                        for j in range(egomotion_num):
                            ...
                            # 光流光度约束
                            curr_bs, curr_h, curr_w, _ = self.pose_net_flow_bwd[egomotion_index][
                                s].get_shape().as_list()
                            scale_factor = tf.cast(tf.constant([curr_w, curr_h], shape=[1, 1, 1, 2]), 'float32')
                            scale_factor = tf.tile(scale_factor, [curr_bs, curr_h, curr_w, 1])
                            self.pose_net_flow_bwd[egomotion_index][s] = self.pose_net_flow_bwd[egomotion_index][
                                                                             s] * scale_factor
            ...
            with tf.name_scope('spatial_loss'):
                ...
                with tf.name_scope('spatial_left_right'):
                    for j in range(self.seq_length):
                        ...
                        self.spatial_warped_image[s][key], self.spatial_warp_mask[s][key], self.spatial_rigid_flow[s][
                            key] = (
                        ...
                        self.spatial_warp_error[s][key], self.spatial_ssim_error[s][
                            key] = self.spatial_image_similarity(
                ...
                with tf.name_scope('left_right_occlusion'):
                    for j in range(self.seq_length):
                        key_lwd = 'lwd-%d' % (j)
                        key_rwd = 'rwd-%d' % (j)
                        rwd2lwd[s][key_lwd] = project.flow_warp(self.spatial_rigid_flow[s][key_rwd],
                                                                self.spatial_rigid_flow[s][key_lwd])
                        lwd2rwd[s][key_rwd] = project.flow_warp(self.spatial_rigid_flow[s][key_lwd],
                                                                self.spatial_rigid_flow[s][key_rwd])
                        # calculate flow diff
                        # # # clip flow to image area
                        # cliped_lwd_flow = self.clip_rigid_flow(lwd_flow)
                        # cliped_rwd_flow = self.clip_rigid_flow(rwd_flow)
                        lwd_flow_diff[s][key_lwd] = tf.abs((rwd2lwd[s][key_lwd] + self.spatial_rigid_flow[s][
                            key_lwd]))  # * (tf.sign(rwd2lwd[s][key_lwd] + self.spatial_rigid_flow[s][key_lwd])+1)/2
                        rwd_flow_diff[s][key_rwd] = tf.abs((lwd2rwd[s][key_rwd] + self.spatial_rigid_flow[s][
                            key_rwd]))  # * (-1*tf.sign(lwd2rwd[s][key_rwd] + self.spatial_rigid_flow[s][key_rwd])+1)/2
                        # build flow consistency condition
                        lwd_consist_bound[s][key_lwd] = 0.04 * self.L2_norm(
                            self.spatial_rigid_flow[s][key_lwd]) * 2 ** s
                        rwd_consist_bound[s][key_rwd] = 0.04 * self.L2_norm(
                            self.spatial_rigid_flow[s][key_rwd]) * 2 ** s
                        ...
                        # SSIM error
                        if self.use_flow_consistency_mask is True:
                            self.spatial_ssim_error[s][key_lwd] *= slim.avg_pool2d(
                                self.spatial_flow_consistency_mask_left[s][key_lwd], 3, 1,
                                'VALID')
                            self.spatial_ssim_error[s][key_rwd] *= slim.avg_pool2d(
                                self.spatial_flow_consistency_mask_right[s][key_rwd], 3, 1,
                                'VALID')
                ...
                with tf.name_scope('left_right_disp_consistency'):
                    for j in range(self.seq_length):
                        ...
                        self.spatial_right2left_disp[s][key_lwd] = project.flow_warp(self.disp_right[j][s],
                                                                                     self.spatial_rigid_flow[s][
                                                                                         key_lwd])
                        self.spatial_left2right_disp[s][key_rwd] = project.flow_warp(self.disp_left[j][s],
                                                                                     self.spatial_rigid_flow[s][
                                                                                         key_rwd])

                        self.spatial_lr_consist_warp_error[s][key_lwd] = tf.abs(
                            self.spatial_right2left_disp[s][key_lwd] - self.disp_left[j][s])
                        self.spatial_lr_consist_warp_error[s][key_rwd] = tf.abs(
                            self.spatial_left2right_disp[s][key_rwd] - self.disp_right[j][s])

                        if self.use_charbonnier_loss is True:
                            self.spatial_lr_consist_warp_error[s][key_lwd] = self.charbonnier_loss(
                                self.spatial_lr_consist_warp_error[s][key_lwd])
                            self.spatial_lr_consist_warp_error[s][key_rwd] = self.charbonnier_loss(
                                self.spatial_lr_consist_warp_error[s][key_rwd])
                        if self.use_flow_consistency_mask is True:
                            self.spatial_lr_consist_warp_error[s][key_lwd] *= \
                            self.spatial_flow_consistency_mask_left[s][key_lwd]
                            self.spatial_lr_consist_warp_error[s][key_rwd] *= \
                            self.spatial_flow_consistency_mask_right[s][key_rwd]

                        self.spatial_lr_consist_warp_loss += tf.reduce_mean(
                            self.spatial_lr_consist_warp_error[s][key_lwd])
                        self.spatial_lr_consist_warp_loss += tf.reduce_mean(
                            self.spatial_lr_consist_warp_error[s][key_rwd])
    ...
    def temporal_image_similarity(self, temporal_warp_image, target_images, sad_loss_kernel=None,
                                  warp_mask=None,
                                  dynamic_mask=None,
                                  occlusion_mask=None):
        # Reconstruction.
    ...
    def spatial_image_similarity(self, spatial_warp_image, target_images, target_images_disp=None,
    ...
    def cal_temporal_occlusion_mask(self, source_flow, target_flow, scale, bata=3):
        ...
        consist_bound = 0.04 * self.L2_norm(target_flow) * 2 ** scale
